...
function viewCart() {
  ...
  const productsInCart = savedProductIds.map(productId => {
...
<template>
  ...
  <div class="container px-5 py-24 mx-auto">
      ...
      <table class="table-auto w-full text-left whitespace-no-wrap">
        ...
        <tbody>
          <tr v-for="(product, index) in cartData" :key="index">
            <td class="px-4 py-3"><a class="block relative h-48 w-80 rounded overflow-hidden">
              <img
                alt="ecommerce"
            ...
            <td class="px-4 py-3 flex gap-5 px-30">
              <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" @click="updateQTY(index)"> + QTY</button>
              <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" @click="removeQTY(index)"> - QTY</button>
            </td>
    ...
    <div class="flex pl-4 mt-4 lg:w-2/3 w-full mx-auto">
      ...
      <button class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded" @click="purchaceNow">Purchace Now</button>
